class SCR_AITakeGadgetInLeftHandCustom : AITaskScripted
{
	...
	override ENodeResult EOnTaskSimulate(AIAgent owner, float dt)
	{
		...
		if (m_bWaiting)
		{
			if (IsItemEquipped(m_ItemEntity))
			{
				...
				SCR_FlashlightComponent flashlight = SCR_FlashlightComponent.Cast(m_ItemEntity.FindComponent(SCR_FlashlightComponent));
				if (!flashlight.IsToggledOn())
					flashlight.ActivateAction();
				Reset();
...
class SCR_AIToggleFlashlightsOnAbortCustom : SCR_AIToggleFlashlights
{
	//------------------------------------------------------------------------------------------------------
	override void OnAbort(AIAgent owner, Node nodeCausingAbort)
	{
