...
class SCR_AIExecuteNavigateMeselfTaskBehavior : SCR_AIBehaviorBase
{
	protected ref SCR_BTParam<SP_Task> m_vTask = new SCR_BTParam<SP_Task>("Task");
	...
	// posWorld - position to observe
	void SCR_AIExecuteNavigateMeselfTaskBehavior(SCR_AIUtilityComponent utility, SCR_AIActivityBase groupActivity, SP_Task Task)
	{
		InitParameters(Task);
		PickedTask = Task;	
		m_sBehaviorTree = "{A670B53B3B00B2D2}AI/BehaviorTrees/SP_AITaskAction-Navigate.bt";
...
class SCR_AINavigateMeselfTaskParameters : SCR_AIGetActionParameters
{
	static ref TStringArray s_aVarsOut = (new SCR_AIExecuteNavigateMeselfTaskBehavior(null, null, null)).GetPortNames();
	override TStringArray GetVariablesOut()
	{
		return s_aVarsOut;
	}
