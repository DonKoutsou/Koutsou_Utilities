...
class SCR_MedicalAvailableItemHintUIInfo : SCR_BaseSupplyItemHintUIInfo
{
	...
	override bool CanBeShown(InventoryItemComponent item)
	{
		...
		return resourceComp.GetConsumer(EResourceGeneratorID.DEFAULT, EResourceType.Medical);
	}
	...
	protected override void GetSupplyAmounts(InventoryItemComponent item, out float supplies, out float maxSupplies = -1)
	{
		...
		SCR_ResourceConsumer consumer = resourceComp.GetConsumer(EResourceGeneratorID.DEFAULT, EResourceType.Medical);
		if (!consumer)
			return;
...
[BaseContainerProps(configRoot: true)]
class SCR_RepairAvailableItemHintUIInfo : SCR_BaseSupplyItemHintUIInfo
{
	[Attribute("0", desc: "If true will not show this hint if the item is howered over in vicinity")]
	protected bool m_bHideIfInvicinity;
	...
	override bool CanBeShown(InventoryItemComponent item)
	{
		...
		return resourceComp.GetConsumer(EResourceGeneratorID.DEFAULT, EResourceType.Repair);
	}
	...
	protected override void GetSupplyAmounts(InventoryItemComponent item, out float supplies, out float maxSupplies = -1)
	{
		...
		SCR_ResourceConsumer consumer = resourceComp.GetConsumer(EResourceGeneratorID.DEFAULT, EResourceType.Repair);
		if (!consumer)
			return;
