...
class SCR_ScenarioFrameworkPluginTrigger: SCR_ScenarioFrameworkPlugin
{
	...
	protected FactionKey m_FactionToSet;
	...
	override void Init(SCR_ScenarioFrameworkLayerBase object)
	{
		...
		if (trigger)
		{
			...
			if (m_sActivatedByThisFaction.IsEmpty())
				trigger.SetOwnerFaction(m_Object.GetFactionKey());
			else if (m_sActivatedByThisFaction != "-1")
				trigger.SetOwnerFaction(m_sActivatedByThisFaction);
			trigger.SetOwnerFaction(m_FactionToSet);
